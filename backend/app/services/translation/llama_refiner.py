"""
Llama Refiner Module
Refines NLLB translations using Llama 3.2 model
"""
import requests
import re
...
def refine_batch_llama(
    ...
    prompt = (
        ...
        "4. Improve naturalness while preserving meaning\n\n"
        f"Refine the following {target_name} texts:\n{combined_text}"
    )
    
    print(f"   ✨ Refining with Llama ({len(nllb_texts)} blocks)...")
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            for i in range(len(nllb_texts)):
                ...
                if match:
                    ...
                    if re.search(r'\b(refined version|more natural|I made|I changed)\b', block_content, re.IGNORECASE):
                        print(f"   ⚠️ Llama Block {i+1}: Rejected English explanation")
                        results[i] = ""
                    else:
                        results[i] = block_content.strip()
                else:
                    print(f"   ⚠️ Llama Block {i+1} not found in response")
            
            # Fallback: use NLLB if Llama failed
            ...
            print(f"   ❌ Llama API Error: {resp.status_code}")
            return nllb_texts
